...
export default function DonorChat() {
  ...
  const otherUserOnline = recipientId ? isUserOnline(recipientId) : false;
  const otherUserLastSeen = recipientId ? getUserLastSeen(recipientId) : null;

  // Fetch distribution to get recipient name and ID
  useEffect(() => {
    const fetchDistribution = async () => {
      ...
      try {
        ...
        if (dist?.recipient) {
          const name = `${dist.recipient.firstName} ${dist.recipient.lastName}`;
          setRecipientName(name);
          setRecipientId(dist.recipient.userID || dist.recipient.id);
        }
      } catch (err) {
