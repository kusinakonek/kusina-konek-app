...
import {
    User, LogOut, Phone, Mail, MapPin, Calendar, ArrowLeft,
    Edit2, Settings, Heart, Globe, Star, ChevronDown, Package, RefreshCw
...
export default function Profile() {
    ...
    const displayBarangay = profileData?.profile?.address?.barangay
    ...
    const hasDonorHistory = donorHistoryStats && (
        (donorHistoryStats.totalDonated && donorHistoryStats.totalDonated > 0) ||
        (donorHistoryStats.availableItems && donorHistoryStats.availableItems > 0)
    );
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <View style={styles.header}>
                ...
                <TouchableOpacity style={styles.headerBtn} onPress={() => router.push('/(tabs)/edit-profile')}>
            ...
            >
                ...
                <View style={styles.profileCard}>
                    ...
                    >
                        ...
                        {switchingRole && <ActivityIndicator size="small" color="#1E88E5" style={{ marginLeft: wp(4) }} />}
                    </TouchableOpacity>
                ...
                {role === 'RECIPIENT' && hasDonorHistory && (
                    <View style={styles.donorHistoryCard}>
                        <View style={styles.donorHistoryHeader}>
                            ...
                            <View style={{ flex: 1 }}>
                                <Text style={styles.donorHistoryTitle}>Your Donor Activity</Text>
                                <Text style={styles.donorHistorySubtitle}>Contributions you made as a donor</Text>
                            </View>
                        </View>
                        ...
                        <View style={styles.statsRow}>
                            <View style={[styles.statBadge, { backgroundColor: '#E8F5E9' }]}>
                                ...
                                <Text style={styles.statValue}>{donorHistoryStats?.totalDonated || 0}</Text>
                            </View>
                            <View style={[styles.statBadge, { backgroundColor: '#FFF8E1' }]}>
                                <Star size={wp(16)} color="#FFC107" fill="#FFC107" />
                                <Text style={styles.statLabel}>Avg Rating</Text>
                                <Text style={styles.statValue}>{donorHistoryStats?.averageRating || 'N/A'}</Text>
                            </View>
                        </View>
                    </View>
                )}
            ...
            >
                <Pressable style={styles.modalOverlay} onPress={() => setShowRolePicker(false)}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalSubtitle}>Choose how you want to use KusinaKonek</Text>

                        <TouchableOpacity
                            style={[styles.roleOption, role === 'DONOR' && styles.roleOptionActive]}
                            onPress={() => handleRoleSwitch('DONOR')}
                        >
                            <View style={[styles.roleOptionIcon, { backgroundColor: '#E8F5E9' }]}>
                                <Heart size={wp(24)} color="#2E7D32" />
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.roleOptionTitle}>Donor</Text>
                                <Text style={styles.roleOptionDesc}>Share food with families in need</Text>
                            </View>
                            {role === 'DONOR' && <View style={styles.roleOptionCheck} />}
                        </TouchableOpacity>

                        <TouchableOpacity
                            style={[styles.roleOption, role === 'RECIPIENT' && styles.roleOptionActive]}
                            onPress={() => handleRoleSwitch('RECIPIENT')}
                        >
                            <View style={[styles.roleOptionIcon, { backgroundColor: '#E3F2FD' }]}>
                                <Package size={wp(24)} color="#1E88E5" />
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.roleOptionTitle}>Recipient</Text>
                                <Text style={styles.roleOptionDesc}>Browse and receive free food</Text>
                            </View>
...
const styles = StyleSheet.create({
    ...
    profileCard: {
        backgroundColor: '#fff', borderRadius: wp(20), overflow: 'hidden', marginBottom: hp(16),
    ...
    avatarContainer: {
        width: wp(80), height: wp(80), borderRadius: wp(40), backgroundColor: '#E8F5E9',
        justifyContent: 'center', alignItems: 'center', borderWidth: 4, borderColor: '#fff',
    },
    roleBadge: {
        flexDirection: 'row', alignItems: 'center', alignSelf: 'center', gap: wp(6),
        backgroundColor: '#E3F2FD', paddingHorizontal: wp(14), paddingVertical: hp(6),
        borderRadius: wp(14), marginTop: hp(8),
    },
    roleDot: { width: wp(8), height: wp(8), borderRadius: wp(4), backgroundColor: '#00C853' },
    roleText: { color: '#1E88E5', fontWeight: '600', fontSize: fp(13) },
    infoSection: { padding: wp(20) },
    infoRow: { flexDirection: 'row', alignItems: 'center', gap: wp(6), marginBottom: hp(2) },
    infoLabel: { fontSize: fp(12), color: '#999' },
    infoValue: { fontSize: fp(16), fontWeight: '600', color: '#1a1a1a', marginLeft: wp(22) },
    statsCard: {
        backgroundColor: '#fff', borderRadius: wp(20), padding: wp(20), marginBottom: hp(16),
        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2,
    },
    statsHeader: { flexDirection: 'row', alignItems: 'center', gap: wp(12), marginBottom: hp(16) },
    statsIconWrap: {
        width: wp(36), height: wp(36), borderRadius: wp(18), backgroundColor: '#FFF8E1',
        justifyContent: 'center', alignItems: 'center',
    },
    statsTitle: { fontSize: fp(16), fontWeight: 'bold', color: '#1a1a1a' },
    statsSubtitle: { fontSize: fp(12), color: '#999', marginTop: hp(2) },
    statsRow: { flexDirection: 'row', gap: wp(12), marginBottom: hp(12), flexWrap: 'wrap' },
    statBadge: { flex: 1, minWidth: isTablet ? wp(140) : wp(120), borderRadius: wp(14), padding: wp(16) },
    statLabel: { fontSize: fp(12), color: '#666', marginTop: hp(8), marginBottom: hp(4) },
    ...
    donorHistoryHeader: { flexDirection: 'row', alignItems: 'center', gap: wp(12), marginBottom: hp(16) },
    donorHistoryIconWrap: {
        width: wp(36), height: wp(36), borderRadius: wp(18), backgroundColor: '#E8F5E9',
        justifyContent: 'center', alignItems: 'center',
    },
    ...
    settingsTitle: { fontSize: fp(16), fontWeight: 'bold', color: '#1a1a1a', marginBottom: hp(16) },
    settingsRow: { flexDirection: 'row', alignItems: 'center', gap: wp(12), paddingVertical: hp(14) },
    settingsRowText: { fontSize: fp(15), color: '#333', fontWeight: '500' },
    divider: { height: 1, backgroundColor: '#f0f0f0' },
    updateBanner: {
        backgroundColor: '#FFF3E0', borderRadius: wp(12), padding: wp(14), marginBottom: hp(16),
    ...
    modalOverlay: {
        ...
        justifyContent: 'center', alignItems: 'center', padding: wp(20),
    },
    modalContent: {
        backgroundColor: '#fff', borderRadius: wp(24), padding: wp(24),
        width: '100%', maxWidth: wp(400),
    },
    modalTitle: { fontSize: fp(20), fontWeight: 'bold', color: '#1a1a1a', marginBottom: hp(4) },
    modalSubtitle: { fontSize: fp(14), color: '#666', marginBottom: hp(20) },
    roleOption: {
        flexDirection: 'row', alignItems: 'center', gap: wp(14),
        padding: wp(16), borderRadius: wp(16), borderWidth: 2, borderColor: '#f0f0f0',
        marginBottom: hp(12),
    },
    ...
    modalCancelBtn: {
        alignItems: 'center', paddingVertical: hp(14), marginTop: hp(4),
    },
