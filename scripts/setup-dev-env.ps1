...
function Get-BackendPortFromEnvFile {
    ...
    try {
        $line = (Get-Content $EnvPath -Raw -ErrorAction Stop)
        $match = [regex]::Match($line, '(?m)^\s*PORT\s*=\s*(\d+)\s*$')
        if ($match.Success) {
...
if ($ipAddress) {
    ...
    $backendPort = (Get-BackendPortFromEnvFile -EnvPath $envFile)
    ...
    if (Test-Path $envFile) {
        ...
        if ($content -match 'EXPO_PUBLIC_API_HOST=') {
            ...
            if ($newContent -match 'EXPO_PUBLIC_API_PORT=') {
                $newContent = $newContent -replace 'EXPO_PUBLIC_API_PORT=.*', "EXPO_PUBLIC_API_PORT=$backendPort"
            }
...
EXPO_PUBLIC_API_PORT=$backendPort
"@
    ...
    # Write Expo env file in apps/mobile (this is the directory where expo runs)
    $mobileEnvPath = Join-Path (Join-Path $rootDir "apps\mobile") ".env"
    $mobileEnvContent = @"
# Auto-generated by scripts/setup-dev-env.ps1
EXPO_PUBLIC_API_HOST=$ipAddress
EXPO_PUBLIC_API_PORT=$backendPort
"@
    $mobileEnvContent | Set-Content $mobileEnvPath -NoNewline
    Write-Host "[+] Wrote apps/mobile/.env for Expo" -ForegroundColor Green
    
    # Check if firewall is open for backend port
    Write-Host ""
    $firewallRule = netsh advfirewall firewall show rule name="Kusina Konek Backend (Port $backendPort)" 2>$null
    if ($LASTEXITCODE -ne 0) {
        Write-Host "⚠️  Firewall WARNING:" -ForegroundColor Yellow
        Write-Host "   Port $backendPort is NOT open in Windows Firewall!" -ForegroundColor Yellow
        ...
        Write-Host "✅ Firewall: Port $backendPort is open" -ForegroundColor Green
    }
    ...
    Write-Host "http://${ipAddress}:$backendPort/api" -ForegroundColor Cyan
    Write-Host ""
